...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        DrawText(currentTurn == FirstPlayer ? "First Player's Turn" : "Second Player's Turn", 10, 10, 20, DARKGRAY);

        if (winner == SecondPlayer)
        {
            DrawText("Second Player Wins! Press R to Restart", 10, 40, 20, DARKGRAY);
        } else if (winner == FirstPlayer)
        {
            DrawText("First Player Wins! Press R to Restart", 10, 40, 20, DARKGRAY);
        }
